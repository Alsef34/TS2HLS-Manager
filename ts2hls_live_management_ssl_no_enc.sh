...
add_user() {
    ...
    for BASE_ID in "${ID_ARRAY[@]}"; do
        ...
        # ffmpeg
        nohup ffmpeg -re -i "$B_URL" -c:v copy -c:a copy -f hls -hls_time 5 -hls_list_size 12 -hls_flags delete_segments+discont_start -max_delay 5000000 "$USER_DIR/$B_ID.m3u8" >"$USER_DIR/ffmpeg-$B_ID.log" 2>&1 &
...
menu() {
    clear
    update_domain_from_nginx
    ts2hls_live_management_ssl_no_enc=$(grep -E '^ts2hls_live_management_ssl_no_enc=' /path/to/ts2hls.sh | cut -d'=' -f2 | tr -d '"')
    echo "========================================================="
    echo " HLS Yönetim Scripti SSL - NO-ENC / Kurulu Sürüm: v$VERSION - Güncel Sürüm: v$ts2hls_live_management_ssl_no_enc"
    echo "========================================================="
